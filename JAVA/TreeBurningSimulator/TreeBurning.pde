...
void setup() {
   ...
   *ANSWER HERE:  col:width and row:height
   */
...
void draw() {
   ...
   *ANSWER HERE: To reduce the amount of times it updates the board so it's not too fast
   */
   ...
   */
   ...
   for(int i = 0; i < ROWS; i++) {
     for(int j = 0; j < COLS; j++) {
       String object = lines[i].substring(j,j+1);
       switch (object) {
         case "@":
           fill(50, 168, 58);
           break;
